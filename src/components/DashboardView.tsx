...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const avgQualityScore =
    agents.length > 0
      ? (
          agents.reduce((sum, a) => sum + (a.qualityScore ?? a.score ?? 0), 0) / agents.length
        ).toFixed(1)
      : '0.0';
            ...
            <div className="space-y-3">
              {topAgents.length === 0 ? (
                <div className="py-8 text-center text-slate-400">
                  <span className="material-symbols-outlined text-3xl mb-1 text-slate-300">group_off</span>
                  <p className="text-xs font-semibold text-slate-600">No agents yet</p>
                  <p className="text-[11px] text-slate-400 mt-0.5">Add agents to see leaderboard rankings</p>
                </div>
              ) : (
            ...
            <button
              onClick={() => {
                if (agents.length > 0) {
                  ...
                  setEditingAgentMetrics(firstAgent);
                  setMetricForm({
                    qualityScore: firstAgent.qualityScore ?? firstAgent.score ?? 85,
                    fatalCount: firstAgent.fatalCount ?? 0,
                    callAuditCount: firstAgent.callAuditCount ?? 1
                  });
                }
              }}
              disabled={agents.length === 0}
              className="px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shadow-xs"
              title="Update agent Quality Score, Fatal Count & Audits"
            >
              ...
              {agents.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-slate-400">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">Real agents created with role "agent" will appear here automatically.</p>
                  </td>
                </tr>
              ) : (
                agents.map((agent) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {publishedSops.length === 0 ? (
            <div className="col-span-full py-12 text-center text-slate-400 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
              <span className="material-symbols-outlined text-4xl mb-2 text-slate-300">menu_book</span>
              <p className="text-sm font-semibold text-slate-700">No process updates yet</p>
              <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                Create and publish standard operating procedures (SOPs) or compliance rubrics to get started.
              </p>
              <button
                onClick={() => onNavigate('new-audit')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {activities.length === 0 ? (
            <div className="col-span-full py-8 text-center text-slate-400 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
              <span className="material-symbols-outlined text-3xl mb-1 text-slate-300">history</span>
              <p className="text-xs font-semibold text-slate-600">No recent activity</p>
              <p className="text-[11px] text-slate-400 mt-0.5">Agent quiz completions and system audit logs will stream here.</p>
            </div>
          ) : (
            activities.map((item) => (
              <div
          ...
          <div className="w-full max-w-md bg-white rounded-3xl p-6 sm:p-7 shadow-2xl border border-slate-200 space-y-5 animate-in zoom-in-95 duration-200">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">
                    {editingAgentMetrics.name} {editingAgentMetrics.email ? `• ${editingAgentMetrics.email}` : `• ${editingAgentMetrics.team}`}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-4 text-xs">
              <div>
                ...
                {agents.length === 0 ? (
                  <div className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-400 text-xs font-semibold">
                    No agents found
                  </div>
                ) : (
                  <select
                    value={editingAgentMetrics.id}
                    onChange={(e) => {
                      const selected = agents.find((a) => a.id === e.target.value);
                      if (selected) {
                        setEditingAgentMetrics(selected);
                        setMetricForm({
                          qualityScore: selected.qualityScore ?? selected.score ?? 85,
                          fatalCount: selected.fatalCount ?? 0,
                          callAuditCount: selected.callAuditCount ?? 1
                        });
                      }
                    }}
                    className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-slate-300 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-100 text-slate-900 font-bold outline-none transition-all cursor-pointer text-xs"
                  >
                    {agents.map((ag) => (
                ...
                {editingAgentMetrics.email && (
                  <p className="text-[11px] text-slate-500 mt-1">
                    Email: <span className="font-semibold text-slate-700">{editingAgentMetrics.email}</span>
                  </p>
                )}
              </div>
